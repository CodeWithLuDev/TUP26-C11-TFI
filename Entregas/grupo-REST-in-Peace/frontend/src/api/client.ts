...
async function request<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(body.error ?? 'No se pudo completar la solicitud', response.status, body.code)
  }
...
export const api = {
  ...
  assisters(token: string) {
    return request<{ assisters: RankedAssister[] }>('/me/stats/assisters', {}, token)
  },
  ...
  submitResult(token: string, matchId: number, payload: SubmitResultPayload) {
    return request(`/me/matches/${matchId}/result`, {
      method: 'POST',
      body: JSON.stringify(payload),
    }, token)
  },
  deleteResult(token: string, matchId: number) {
    return request(`/me/matches/${matchId}/result`, { method: 'DELETE' }, token)
  },
}
